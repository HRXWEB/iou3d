...
void testRotationDirection() {
    ...
    std::cout << "原始框 (length=3, width=1):" << std::endl;
    ...
    std::cout << "\n期望结果：原来沿x轴的长边(3)现在应该沿z轴方向" << std::endl;
    std::cout << "验证：检查z方向的范围应该约为3，x方向的范围应该约为1" << std::endl;
    ...
    if (std::abs(z_range - 3.0f) < 0.1f && std::abs(x_range - 1.0f) < 0.1f) {
